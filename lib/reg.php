...
if ((empty($i['opt']['core_version']) || SYSTEM_VER != $i['opt']['core_version']) && !defined('SYSTEM_NO_CHECK_VER')) {
    ...
    if (file_exists(SYSTEM_ROOT . '/setup/update' . $i['opt']['core_version'] . 'to' . SYSTEM_VER . '.php')) {
        ...
        $sql = $m->db_type() === 'sqlite'
            ? "INSERT OR REPLACE INTO `" . DB_PREFIX . "options` (`name`, `value`) VALUES ('core_version','" . SYSTEM_VER . "');"
            : "INSERT INTO `" . DB_PREFIX . "options` (`name`, `value`) VALUES ('core_version','" . SYSTEM_VER . "') ON DUPLICATE KEY UPDATE `value` = '" . SYSTEM_VER . "';";
        $m->query($sql);
    }
}
